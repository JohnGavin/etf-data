  ...
  if (dir.exists(store_path)) {
    ...
    if (!is.null(tar_meta(store = store_path))) {
      ...
      if (nrow(meta) > 0) {
        ...
        knitr::kable(meta, caption = "Targets Pipeline Metadata")
      }
    }
    
    # 2. Network Visualization
    # Note: static image capture might be needed for some PDF outputs, 
    # but for HTML this generates an interactive widget.
    cat("### Pipeline Network\n")
    tryCatch({
      tar_visnetwork(store = store_path, targets_only = TRUE)
    }, error = function(e) {
      message("Could not generate visNetwork: ", e$message)
    })
    
  } else {
    message("Targets store not found at ", store_path, ". Metadata visualization skipped.")
  }
}
```
